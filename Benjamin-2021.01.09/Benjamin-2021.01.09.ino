...
#define MAXPOWER 255 * 0.8  //モーターの最大パワー
#define MINPOWER 255 * 0.3  //モーターの最小パワー (動き始める値)
...
const uint8_t linePin[8] = {PA1, PA0, PC1, PC2, PB0, PA4, PC0, PC3};  //ラインセンサーのピン
double power = MINPOWER, moveDeg = 1000.0, pre_moveDeg = 1000.0;    //モーターのパワー(0～255)
byte line_x = 0, line_y = 0;
bool isKick = false;
...
float getGyroPID() {
    const float Kp = 1.2, Ki = 0.2, Kd = 0.05, Imax = 50;
...
void move(double deg, float diff) {
    const double powerChange = 10.0, MDeg[4] = {50.0, 130.0, 230.0, 310.0};  //パワーの変化率、モーターの付いてる角度
...
void avoidLine() {
    const double powerChange = 20.0;
    static uint32_t pre_time;
    static bool isWait = false, isGoForward = false;
    //static byte pre_line_x, pre_line_y, firstNum_x, firstNum_y, direction_x, direction_y;
    double x = 0.0, y = 0.0, degDiff;
    if (judgeIsLine() || lineState != 0) {
        switch (lineState) {
            case 0: //初めてラインを踏んだ場合
                ...
                /*  firstNum_x = line_x;
                    firstNum_y = line_y;
                    if ((line_x & B0011) != 0) direction_x = B0011;
                    else if ((line_x & B1100) != 0) direction_x = B1100;
                    if ((line_y & B0011) != 0) direction_y = B0011;
                    else if ((line_y & B1100) != 0) direction_y = B1100;*/
                lineState = 1;      // 1に進む
                break;
            case 1: //
                ...
                break;
            case 2: // ライン上にいる場合
                //if ((power + powerChange) < MAXPOWER) power += powerChange; // モーターの負担を減らすためpowerを徐々に上げてく
                if (isGoForward) x -= 1.5;
                if ((line_x & B0011) != 0) x -= 1.0;
                if ((line_x & B1100) != 0) x += 1.0;
                if ((line_y & B0011) != 0) y -= 1.0;
                if ((line_x & B1100) != 0) y += 1.0;
                moveDeg = degrees(atan2(y, x));
                if (judgeIsLine() == false) {
                    /*  if (firstNum_x != 0) { //x(縦)方向のラインが反応してたとき
                        if ((pre_line_X == firstNum_x) || (pre_line_X == direction_x)) {
                            firstNum_x = 0; //firstNum_xをリセット
                        }
                        }
                        if (firstNum_y != 0) {  //y(横)方向のラインが反応してたとき
                        if (pre_line_y == firstNum_y) || (pre_line_y == direction_y)) {
                            firstNum_y = 0; //firstNum_yをリセット
                        }
                        }
                        if (firstNum_x == 0 && firstNum_y == 0) {*/   //どちらもリセットされたら
                    lineState = 3;
                    //}
                }
            case 3: // ライン上にいない場合
                getBall();
                degDiff = fabs(pre_moveDeg - ballDeg);   // 動いた角度とボールの角度の差の絶対値(避けた後なのでpre_moveDegはラインのある方向と逆の角度になる)をとる
                if (degDiff > 180) degDiff = 360 - degDiff;

                if ((ballDeg != 1000.0) && (degDiff < 90)) { // 差の絶対値が90°未満だったらボールを追いかける
                    if (isWait) isWait = false; //もしisWaitがtrueになってたらfalseに戻す
                    lineState = 4;  // 4に進む
                } else {            // 差の絶対値が90°以上だったら待つ
                    if ((power - powerChange) > MINPOWER) {
                        power -= powerChange; // モーターの負担を減らすためpowerを徐々に下げてく
                        if (judgeIsLine()) lineState = 0;   // ラインを踏んだら0に戻る
                    } else {
                        if (!isWait) {
                            pre_time = millis();
                            moveDeg = 1000.0;
