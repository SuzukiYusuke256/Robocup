...
#define MAXPOWER 255 * 0.7  //モーターの最大パワー
#define MINPOWER 255 * 0.5  //モーターの最小パワー (動き始める値)
...
Display oled = Display(&SPI, OLED_CS, OLED_DC, OLED_RST); //use the hardware SPI pins
...
void loop() {
    ...
    if (isKick) {
        if ((millis() - kickTime) > 3000) {
            isKick = false;
        } else if ((millis() - kickTime) > 50) {
...
void getBall() {
    ...
    for (byte i = 0; i < 12; i++) {
        value[i] = (value[i] * 0.5) + (pulseWidth[i] * 0.5); //値を安定させる
    ...
    if ((x == 0) && (y == 0)) { // ボールが見つからない場合
        ...
        ballDistance = (int)(round(ballDistance / 60) * 5);   //平均をとって5単位にする
    }
    if (isDisplay_ball) {
        oled.fillScreen(BLACK);
        oled.setCursor(0, 0);
...
void getLine() {
    const int threshold[8] = {750, 750, 750, 750, 750, 750, 750, 750};
    int line[8] = {0};
    for (byte i = 0; i < 8; i++) {
...
void move(double deg, float diff) {
    const double powerChange = 5.0, MDeg[4] = {50.0, 130.0, 230.0, 310.0};  //パワーの変化率、モーターの付いてる角度
...
void wrapAroundBall() {
    const double plusDeg = 42.0, goForwardDeg = 10.0, ratio = pow(10, (ballDistance / 85) - 1);
    if (ballDeg == 1000.0) {
        moveDeg == 1000.0;
    } else if (judgeIsHoldBall()) {
        moveDeg = 0;
    } else if (-goForwardDeg <= ballDeg && ballDeg <= 0) {
        moveDeg = 0;
    } else if (ballDeg > 0) {
        moveDeg = ballDeg + (sqrt((ballDeg * plusDeg) + 600) * ratio);
    } else if (ballDeg < -goForwardDeg) {
        moveDeg = ballDeg - (sqrt((-ballDeg * plusDeg) + 540) * ratio);
    } else {
        moveDeg = 1000.0;
    }
}
...
byte displayButton() {  //ボタンを 0:押してない 1;短押し(longPressTime[ms]未満) 2:長押し]
    const int longPressTime = 800;
    uint32_t preTime;
    if (digitalRead(BUTTON_PIN) == HIGH) {
        preTime = millis();
        while (digitalRead(BUTTON_PIN) == HIGH) {
            if ((millis() - preTime) < longPressTime) oled.buttonState(YELLOW);
            else oled.buttonState(RED);
            delay(20);
        }
        if ((millis() - preTime) < longPressTime) return 1;  //短押しのとき
        else return 2;  //長押しのとき
    } else {
        oled.buttonState(BLUE);
        return 0;
    }
}
